...
let array = [
    { id: 0, firstName: 'Fred', lastName: 'Smith', jobTitle: 'Cashier', address: '500 Somewhere Lane', color: 'blue' },
    { id: 1, firstName: 'Bob', lastName: 'Boot', jobTitle: 'Manager', address: '600 Somewhere Lane', color: 'Red' },
    { id: 3, firstName: 'Calvin', lastName: 'snek', jobTitle: 'Secretary', address: '800 Somewhere Lane', color: 'Green' },
    { id: 4, firstName: 'Glen', lastName: 'Oz', jobTitle: 'CTO', address: '900 Somewhere Lane', color: 'Teal' },
    { id: 5, firstName: 'Austin', lastName: 'ALfonz', jobTitle: 'CEO', address: '666 Somewhere Lane', color: 'Orange' }
]

const createEmployeeTable = () => {
    db.run("CREATE TABLE IF NOT EXISTS employees (id INT, firstName TEXT, lastName TEXT, jobtitle TEXT, address TEXT, color TEXT)");
};
createEmployeeTable();
...
db.all("SELECT * FROM employees", (err, allRows) => {
    allRows.forEach(each => {
        console.log(each.address, each.first + ' ' + each.last);
    });
});